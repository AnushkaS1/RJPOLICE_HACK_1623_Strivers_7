...
def get_transactions():
    ...
    # Limit the response to 10 transactions
    ...
    for _ in range(number_of_transactions):
        ...
        amount_to_debit = round(random.uniform(10.0, 1000.0), 2)
        ...
        # Introduce a probability (e.g., 30%) for generating fake transactions
        if random.random() < 0.3:
            # Add fake transactions with the same sender in a short time frame
            for _ in range(random.randint(2, 5)):
                fake_recipient = random.choice([u for u in users if u != sender])
                fake_timestamp_str = (start_time + datetime.timedelta(seconds=random.randint(1, 30))).strftime('%Y-%m-%dT%H:%M:%S')
                fake_transaction = {
                    "transaction_id": generate_transaction_id(),
                    "sender_user_id": sender["user_id"],
                    "sender_name": sender["name"],
                    "sender_account_number": sender["account_number"],
                    "sender_kyc_info": sender["kyc_info"],
                    "recipient_user_id": fake_recipient["user_id"],
                    "recipient_name": fake_recipient["name"],
                    "recipient_account_number": fake_recipient["account_number"],
                    "recipient_kyc_info": fake_recipient["kyc_info"],
                    "amount": amount_to_debit,
                    "timestamp": fake_timestamp_str,
                    "place": f"{random.choice(['ATM', 'Online', 'Store'])} {random.randint(1, 10)}",
                    "type": "Debit",
                    # Add more transaction details as needed
                }
